...
study = 0  # 0 ='Calculated Total tCO2e'; 1 ='Calculated tCO2e_per_m2'
save_visu = False
...
"""
full_df = dataframe_from_dict(nb_dict, disp=False)
X_df = dataframe_from_dict_keys(nb_dict, FEATURES_NAMES, disp=False)
x_qtt_df = dataframe_from_dict_keys(nb_dict, INT_FEATURES, disp=False)
x_qlt_df = dataframe_from_dict_keys(nb_dict, STR_FEATURES, disp=False) #scaling issue / concatenate df
y_df = dataframe_from_dict_keys(nb_dict, OUTPUT_NAMES, disp=False)
"""
...
#X = create_polynomial_features(X_df, FEATURES_NAMES, powerdict)
